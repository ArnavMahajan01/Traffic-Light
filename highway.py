...
sim.create_roads([
    ...
    *curve_road(SOUTH_RIGHT_OUT, SOUTH_RIGHT_MIDDLE,
                (-a, b-c-c/2), resolution=n),
    *curve_road(NORTH_RIGHT_MIDDLE, NORTH_RIGHT_OUT,
                (-a, -b+c+c/2), resolution=n),
    *curve_road(NORTH_RIGHT_IN, NORTH_LEFT, (a, -b+c/2), resolution=n),

    *curve_road(SOUTH_RIGHT, SOUTH_RIGHT_TURN,
                (SOUTH_RIGHT[0], SOUTH_MIDDLE[1]), resolution=n),
    *curve_road(SOUTH_LEFT_MIDDLE, SOUTH_RIGHT_TURN,
                (SOUTH_RIGHT[0], SOUTH_MIDDLE[1]), resolution=n),

    *curve_road(NORTH_RIGHT_TURN, NORTH_LEFT,
                (NORTH_LEFT[0], NORTH_MIDDLE[1]), resolution=n),
    *curve_road(NORTH_RIGHT_TURN, NORTH_LEFT_MIDDLE,
                (NORTH_LEFT[0], NORTH_MIDDLE[1]), resolution=n),
    ...
    *curve_road(NORTH_LEFT_OUT, NORTH_LEFT_MIDDLE,
                (a, -b+c+c/2), resolution=n),
    *curve_road(NORTH_RIGHT, NORTH_LEFT_IN, (-a, -b+c/2), resolution=n),

    *curve_road(SOUTH_LEFT_TURN, SOUTH_LEFT,
                (SOUTH_LEFT[0], SOUTH_MIDDLE[1]), resolution=n),
    *curve_road(SOUTH_LEFT_TURN, SOUTH_RIGHT_MIDDLE,
                (SOUTH_LEFT[0], SOUTH_MIDDLE[1]), resolution=n),

    *curve_road(NORTH_RIGHT, NORTH_LEFT_TURN,
                (NORTH_RIGHT[0], NORTH_MIDDLE[1]), resolution=n),
    *curve_road(NORTH_RIGHT_MIDDLE, NORTH_LEFT_TURN,
                (NORTH_RIGHT[0], NORTH_MIDDLE[1]), resolution=n),
...
sim.create_gen({
    ...
    'vehicles': [
        ...
        [3, {'path': [1, 23, *road(26+13*n), 16, *
                      road(26+2*n), 14, 15, *road(26+3*n), 11]}],
        [1, {'path': [1, 23, *road(26+13*n), 16, *road(26+15*n), 25, 7]}],

        [2, {'path': [8, *road(26+4*n), 22, 3]}],
        [4, {'path': [8, *road(26), 12, 13, *road(26+n),
                      16, *road(26+2*n), 14, 15, *road(26+3*n), 11]}],
        [2, {'path': [8, *road(26), 12, 13, *road(26+n),
                      16, *road(26+15*n), 25, 7]}],


        [5, {'path': [5, 24, *road(26+6*n), 11]}],
        [3, {'path': [5, 24, *road(26+7*n), 21, *
                      road(26+9*n), 20, 19, *road(26+8*n), 9]}],
        [1, {'path': [5, 24, *road(26+7*n), 21, *road(26+5*n), 22, 3]}],

        [2, {'path': [10, *road(26+14*n), 25, 7]}],
        [4, {'path': [10, *road(26+11*n), 17, 18, *road(26+10*n),
                      21, *road(26+9*n), 20, 19, *road(26+8*n), 9]}],
        [2, {'path': [10, *road(26+11*n), 17, 18, *
                      road(26+10*n), 21, *road(26+5*n), 22, 3]}],
    ]
})
sim.create_signal([[2], [1]])
# Start simulation
win = Window(sim)
win.zoom = 6
